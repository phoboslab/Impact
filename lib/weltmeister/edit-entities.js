...
wm.EditEntities = ig.Class.extend({
	...
	init: function( div ) {
		...
		this.importEntityClass( wm.entityFiles );		
	...
	importEntityClass: function( files ) {		
		var unloadedClasses = [];
		for( var i = 0; i < files.length; i++ ) {
			var className = this.fileNameToClassName(files[i]);
			...
			if( className && ig.global[className] ) {				
				...
				unloadedClasses.push( files[i] + ' (expected name: ' + className + ')' );
			}
		}
	...
	cloneSelectedEntity: function() {
		...
		var settings = this.selectedEntity.settings;
	...
	scaleSelectedEntity: function( x, y ) {
		...
		var w = ( x / this.gridSize ).round() * this.gridSize - this.selectedEntity.pos.x;
		...
		if( scale == 'n' ) {
			var h = this.selectedEntity.pos.y - ( y / this.gridSize ).round() * this.gridSize;
			this.selectedEntity.size.y += h;
			this.selectedEntity.pos.y -= h;
		}
		else if( scale == 's' ) {
			var h = ( y / this.gridSize ).round() * this.gridSize - this.selectedEntity.pos.y;
			this.selectedEntity.size.y = h;
		}
		else if( scale == 'e' ) {
			var w = ( x / this.gridSize ).round() * this.gridSize - this.selectedEntity.pos.x;
			...
			var w = this.selectedEntity.pos.x - ( x / this.gridSize ).round() * this.gridSize;
			this.selectedEntity.size.x += w;
	...
	setEntitySetting: function( ev ) {
		...
		if( key == 'x' ) {
			this.selectedEntity.pos.x = value.round();
		}
		else if( key == 'y' ) {
			this.selectedEntity.pos.y = value.round();
		}
		else {
	...
	showMenu: function( x, y ) {
		this.selector.pos = { 
			x: ( (x + ig.editor.screen.x) / this.gridSize ).round() * this.gridSize, 
			y: ( (y + ig.editor.screen.y) / this.gridSize ).round() * this.gridSize
		};
	...
	toggleVisibility: function() {
		this.visible ^= 1;
		this.div.children('.visible').html( this.visible ? '&#x25a3;' : '&#x25fb;' );
		ig.game.draw();
	},
